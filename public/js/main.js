...
async function loadAllStudents() {
    ...
    try {
        ...
        const response = await fetch(`/api/students/all?t=${timestamp}`, {
            method: 'GET',
            headers: {
...
async function extractDynamicData() {
    ...
    try {
        const timestamp = new Date().getTime();
        const streamsResponse = await fetch(`/api/students/streams?t=${timestamp}`, {
            headers: {
                'Cache-Control': 'no-cache, no-store, must-revalidate',
                'Pragma': 'no-cache'
            }
    ...
    try {
        const timestamp = new Date().getTime();
        const languagesResponse = await fetch(`/api/subjects/languages?t=${timestamp}`, {
            headers: {
                'Cache-Control': 'no-cache, no-store, must-revalidate',
                'Pragma': 'no-cache'
            }
    ...
    try {
        const timestamp = new Date().getTime();
        const electivesResponse = await fetch(`/api/students/subjects/electives?t=${timestamp}`, {
            headers: {
                'Cache-Control': 'no-cache, no-store, must-revalidate',
                'Pragma': 'no-cache'
            }
